...
yy_symbol_value_print (FILE *yyo,
                       yysymbol_kind_t yykind, YYSTYPE const * const yyvaluep, char **ast)
{
  FILE *yyoutput = yyo;
  YY_USE (yyoutput);
  YY_USE (ast);
...
yydestruct (const char *yymsg,
            yysymbol_kind_t yykind, YYSTYPE *yyvaluep, char **ast)
{
  YY_USE (yyvaluep);
  YY_USE (ast);
  if (!yymsg)
...
#line 32 "src/sysy.y"
            {
    *ast = strdup((yyvsp[0].str_val));
  }
#line 1084 "src/sysy.tab.c"
    break;

  case 3: /* FuncDef: FuncType IDENT '(' ')' Block  */
#line 38 "src/sysy.y"
                                 {
    char *type = (yyvsp[-4].str_val);
    char *ident = (yyvsp[-3].str_val);
    char *block = (yyvsp[0].str_val);
    char *result = malloc(strlen(type) + strlen(ident) + strlen(block) + 5);
    sprintf(result, "%s %s() %s", type, ident, block);
    (yyval.str_val) = result;
  }
#line 1097 "src/sysy.tab.c"
    break;
...
#line 73 "src/sysy.y"
    ...
    sprintf(result, "%d", (yyvsp[0].int_val));
    (yyval.str_val) = result;
  }
#line 1137 "src/sysy.tab.c"
    break;


#line 1141 "src/sysy.tab.c"
